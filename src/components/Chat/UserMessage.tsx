  ...
  return (
    <div className="w-full flex justify-end">
        <div className="w-fit max-w-[75%] flex flex-col items-end">
            <motion.div 
                {...animationProps} 
                className="brand-gradient text-white rounded-2xl rounded-br-lg"
            >
                {/* Content Section */}
                        ...
                        <div className="flex flex-col gap-2 p-3 bg-black/10 rounded-lg border border-white/20">
                            {attachments.map((attachment, index) => (
                                <div key={index} className="flex items-center gap-2">
                                    <FileIcon filename={attachment.name} className="flex-shrink-0 w-5 h-5 text-white/80" />
                                    <span className="truncate text-sm font-medium text-white" title={attachment.name}>{attachment.name}</span>
                                </div>
                            ))}
                        </div>
                    )}
