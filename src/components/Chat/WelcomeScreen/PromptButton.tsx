...
export const PromptButton: React.FC<PromptButtonProps> = ({ icon, text, onClick, color = 'slate' }) => {
    ...
    return (
        <motion.button
            ...
            whileTap={{ scale: 0.95 }}
        >
            <span className="flex items-center justify-center w-8 h-8 rounded-full bg-white/40 dark:bg-black/10 text-lg filter drop-shadow-sm group-hover:scale-110 transition-transform duration-300">
                {icon}
            </span>
