...
// Returns raw Tailwind classes for desktop gradients
const getFileDesktopClasses = (file: File): { bg: string, text: string, label: string } => {
    const mime = file.type;
    const name = file.name.toLowerCase();
    const ext = name.split('.').pop()?.toUpperCase().substring(0, 4) || 'FILE';

    if (mime.startsWith('image/')) return { bg: 'md:bg-gradient-to-br md:from-blue-400 md:to-blue-600', text: 'md:text-white', label: 'IMG' };
    if (mime.startsWith('video/')) return { bg: 'md:bg-gradient-to-br md:from-fuchsia-500 md:to-purple-600', text: 'md:text-white', label: 'VID' };
    if (mime.startsWith('audio/')) return { bg: 'md:bg-gradient-to-br md:from-amber-400 md:to-orange-500', text: 'md:text-white', label: 'AUD' };
    if (mime === 'application/pdf') return { bg: 'md:bg-gradient-to-br md:from-red-400 md:to-red-600', text: 'md:text-white', label: 'PDF' };
    
    if (name.endsWith('.zip') || name.endsWith('.rar') || name.endsWith('.7z') || name.endsWith('.tar') || name.endsWith('.gz')) {
        return { bg: 'md:bg-gradient-to-br md:from-yellow-500 md:to-yellow-600', text: 'md:text-white', label: 'ZIP' };
    }
    ...
    if (codeExtensions.some(e => name.endsWith('.' + e))) {
        return { bg: 'md:bg-gradient-to-br md:from-slate-700 md:to-slate-900', text: 'md:text-blue-200', label: ext };
    }
    
    return { bg: 'md:bg-gradient-to-br md:from-gray-400 md:to-gray-500', text: 'md:text-gray-100', label: ext };
};
...
export const AttachedFilePreview: React.FC<AttachedFilePreviewProps> = ({ file, onRemove, onPreview, progress, error }) => {
    ...
    const desktopVisuals = getFileDesktopClasses(file);
        ...
        >
            ...
            <div className="relative w-full h-full p-0 md:p-1 overflow-hidden rounded-xl">
                {previewUrl ? (
                    ...
                    <div className={`w-full h-full rounded-lg flex flex-col items-center justify-center bg-gray-100 dark:bg-white/10 ${desktopVisuals.bg} md:shadow-inner border border-gray-100 dark:border-white/5 md:border-0`}>
                        <FileIcon filename={file.name} className={`w-6 h-6 text-slate-400 dark:text-slate-500 ${desktopVisuals.text} opacity-90 drop-shadow-sm transform group-hover:scale-110 transition-transform duration-300`} />
                        <span className="text-[8px] font-bold text-slate-500 dark:text-slate-400 md:text-white/90 mt-0.5 uppercase tracking-wide drop-shadow-sm">{desktopVisuals.label}</span>
                    </div>
                )}
