...
export const AttachedFilePreview: React.FC<AttachedFilePreviewProps> = ({ file, onRemove, progress, error }) => {
        ...
        <AnimatePresence>
            {(isProcessing || hasFailed) && (
                <motion.div
                    ...
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                    className="absolute inset-0 flex flex-col items-center justify-center p-1 text-center text-white bg-black/60 rounded-md backdrop-blur-[1px]"
                >
                    {hasFailed ? (
                        <>
    ...
    return (
        <motion.div 
            ...
            className="relative group w-full bg-white dark:bg-white/5 border border-gray-200 dark:border-white/5 p-2 rounded-xl flex items-center gap-3 shadow-sm"
        >
            {previewUrl ? (
            ...
            <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-gray-700 dark:text-slate-200 truncate leading-tight" title={file.name}>{file.name}</p>
                <div className="flex items-center gap-2 mt-0.5">
                    <p className="text-xs text-gray-500 dark:text-slate-400 font-medium">{fileTypeLabel}</p>
                    {hasFailed && <span className="text-xs text-red-500 font-medium">• Failed</span>}
                </div>
            </div>
            <button
                type="button"
            ...
            >
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4"><path d="M6.28 5.22a.75.75 0 0 0-1.06 1.06L8.94 10l-2.72 2.72a.75.75 0 1 0 1.06 1.06L10 11.06l2.72 2.72a.75.75 0 1 0 1.06-1.06L11.06 10l2.72-2.72a.75.75 0 0 0-1.06-1.06L10 8.94 6.28 5.22Z" /></svg>
            </button>
        </motion.div>
    );
};
