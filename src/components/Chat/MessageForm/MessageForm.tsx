...
export const MessageForm = forwardRef<MessageFormHandle, {
  ...
  const sendButtonClasses = "flex-shrink-0 w-9 h-9 rounded-full flex items-center justify-center transition-all duration-200 ease-in-out";
  const isSendButtonActive = hasInput && !isLoading && !logic.isEnhancing && !isProcessingFiles;
  
  const sendButtonStateClasses = isLoading 
    ? 'bg-red-500 text-white shadow-lg hover:bg-red-400 active:scale-90'
    : (isSendButtonActive
        ? 'text-white shadow-lg bg-gradient-to-br from-[color:var(--primary-glow)] to-[color:var(--accent-glow)] hover:opacity-90 active:scale-90'
  ...
  return (
    <div className="fixed bottom-0 inset-x-0 z-50 flex justify-center pb-4 sm:pb-6 pointer-events-none">
        ...
        >
            ...
            <div className={`flex items-end gap-2 ${logic.isExpanded ? 'flex-col' : ''}`}>
                <div className="flex-grow w-full">
                    <div ref={logic.inputRef} contentEditable={!logic.isEnhancing} onInput={(e) => logic.setInputValue(e.currentTarget.innerText)} onKeyDown={logic.handleKeyDown} onPaste={logic.handlePaste} aria-label="Chat input" role="textbox" data-placeholder={logic.isRecording ? 'Listening...' : (isLoading ? "Generating response..." : "Ask anything, or drop a file...")} className={`content-editable-input w-full px-2 py-1.5 bg-transparent focus:outline-none ${logic.isEnhancing ? 'opacity-50 cursor-not-allowed' : ''}`} style={{ minHeight: '36px', maxHeight: '192px', transition: 'height 0.2s ease-in-out' }} />
                </div>
                
                <div className={`w-full flex items-center justify-between gap-2 ${logic.isExpanded ? 'pt-2 border-t border-[color:var(--surface-border)]' : ''}`}>
                    <div className="flex items-center gap-1">
                        <div className="relative">
                            <button ref={logic.attachButtonRef} type="button" onClick={() => logic.setIsUploadMenuOpen(p => !p)} aria-label="Attach files or folder" title="Attach files or folder" disabled={logic.isEnhancing} className="flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-[color:var(--text-secondary)] hover:bg-black/5 dark:hover:bg-white/5 disabled:cursor-not-allowed disabled:opacity-50 transition-colors">
                        ...
                        <AnimatePresence>
                        {logic.isSupported && (
                            <motion.button initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} type="button" onClick={logic.handleMicClick} disabled={isLoading || logic.isEnhancing} aria-label={logic.isRecording ? 'Stop recording' : 'Start recording'} title={logic.isRecording ? 'Stop recording' : 'Start recording'} className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-[color:var(--text-secondary)] hover:bg-black/5 dark:hover:bg-white/5 disabled:cursor-not-allowed disabled:opacity-50 transition-all ${logic.isRecording ? 'bg-red-500/20 !text-red-500' : ''}`}>
                        ...
                        <ModeToggle isAgentMode={isAgentMode} onToggle={setIsAgentMode} disabled={isLoading}/>
                    </div>

                    <div className="flex items-center gap-1">
                        <AnimatePresence>
                        {hasText && (
                            <motion.button initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} type="button" onClick={logic.handleEnhancePrompt} disabled={logic.isEnhancing} aria-label="Enhance prompt" title="Enhance prompt" className="flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-[color:var(--text-secondary)] hover:bg-black/5 dark:hover:bg-white/5 disabled:cursor-not-allowed disabled:opacity-50 transition-all">
