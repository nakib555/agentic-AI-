...
export const MessageForm = forwardRef<MessageFormHandle, {
  ...
  const sendButtonStateClasses = (hasInput && !isLoading && !logic.isEnhancing && !isProcessingFiles)
    ? 'bg-primary text-primary-content shadow-md hover:bg-primary-focus active:scale-90'
    : 'bg-ui-200 text-text-muted';

  return (
    <motion.form 
        className={`bg-ui-100/30 backdrop-blur-xl border border-color shadow-lg flex flex-col p-2`} 
    ...
    >
        ...
        <div className="flex items-end gap-2">
            ...
            <div className="relative">
                <button ref={logic.attachButtonRef} type="button" onClick={() => logic.setIsUploadMenuOpen(p => !p)} aria-label="Attach files or folder" title="Attach files or folder" disabled={isLoading || logic.isEnhancing} className="flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-text-secondary hover:bg-ui-200/50 disabled:cursor-not-allowed disabled:opacity-50 transition-colors">
