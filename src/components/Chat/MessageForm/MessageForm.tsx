...
export const MessageForm = forwardRef<MessageFormHandle, MessageFormProps>(
  ({ onSubmit, isLoading, onCancel, isAgentMode, setIsAgentMode, messages }, ref) => {
    ...
    const isProcessingFiles = logic.processedFiles.some(
      (f) => f.progress < 100 && !f.error
    );

    const hasInput =
      logic.inputValue.trim().length > 0 || logic.processedFiles.length > 0;
    ...
    const isSendButtonActive =
      hasInput && !isLoading && !logic.isEnhancing && !isProcessingFiles;
    ...
    return (
      <div>
        <form onSubmit={logic.handleSubmit}>
          <motion.div
            className="bg-theme-bg-light dark:bg-[#121212] border border-[rgb(206,217,239)] dark:border-slate-700/50 flex flex-col p-2 rounded-2xl"
          ...
          >
            ...
            <AnimatePresence>
              {logic.proactiveSuggestions.length > 0 && (
                <ProactiveAssistance
                  suggestions={logic.proactiveSuggestions}
                  onSuggestionClick={logic.handleSuggestionClick}
                />
              )}
            </AnimatePresence>
            ...
            <input
              ...
              className="hidden"
              multiple
              // Correct safe directory attributes
              webkitdirectory=""
              directory=""
            />
            ...
            <div className="relative flex-grow">
              ...
              <div
                ref={logic.inputRef}
                contentEditable={!logic.isEnhancing}
                onInput={(e) =>
                  logic.setInputValue(e.currentTarget.textContent ?? '')
                }
                ...
                role="textbox"
                className={`content-editable-input w-full px-2 py-1.5 bg-transparent text-gray-900 dark:text-slate-200 focus:outline-none relative z-10 ${
                  logic.isEnhancing ? 'opacity-50 cursor-not-allowed' : ''
                }`}
                style={{
                  minHeight: '56px',
                  maxHeight: '192px',
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="relative">
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      logic.setIsUploadMenuOpen((p: boolean) => !p)
                    }
                    disabled={logic.isEnhancing}
                    aria-label="Attach files or folder"
                    title="Attach files or folder"
                    className="flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-gray-600 dark:text-slate-300 hover:bg-black/5 dark:hover:bg-white/5 disabled:opacity-50 transition-colors"
                  >
                    <svg
                      ...
                      className="w-5 h-5"
                    >
                      <path d="M10 3a.75.75 0 01.75.75v5.5h5.5a.75.75 0 010 1.5h-5.5v5.5a.75.75 0 01-1.5 0v-5.5H3.75a.75.75 0 010-1.5h5.5V3.75A.75.75 0 0110 3z" />
                    </svg>
                  </button>
                    ...
                    {logic.isUploadMenuOpen && (
                      <UploadMenu
                        ...
                        onFileClick={() => {
                          ...
                          logic.setIsUploadMenuOpen(false);
                        }}
                        onFolderClick={() => {
                          logic.folderInputRef.current?.click();
                          logic.setIsUploadMenuOpen(false);
                        }}
                      />
                    )}
                ...
                <AnimatePresence>
                  {logic.isSupported && (
                    <motion.button
                      ...
                      disabled={isLoading || logic.isEnhancing}
                      aria-label={
                        logic.isRecording ? 'Stop recording' : 'Start recording'
                      }
                      className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-gray-600 dark:text-slate-300 hover:bg-black/5 dark:hover:bg-white/5 disabled:opacity-50 transition-all ${
                        logic.isRecording ? 'bg-red-500/20 !text-red-500' : ''
                      }`}
                    >
                      {logic.isRecording ? (
                        <motion.div
                        ...
                        >
                          <svg
                            ...
                            className="w-5 h-5"
                          >
                            <path d="M5 3.5A1.5 1.5 0 0 1 6.5 2h7A1.5 1.5 0 0 1 15 3.5v9a1.5 1.5 0 0 1-1.5 1.5h-7A1.5 1.5 0 0 1 5 12.5v-9Z" />
                          </svg>
                        </motion.div>
                      ) : (
                        <svg
                        ...
                        >
                          <path
                            ...
                            d="M12 18.75a6 6 0 0 0 6-6v-1.5m-6 7.5a6 6 0 0 1-6-6v-1.5m12 0v-1.5a6 6 0 0 0-6-6v0a6 6 0 0 0-6 6v1.5m6 7.5v3.75m-3.75 0h7.5"
                          />
                        </svg>
                      )}
                    </motion.button>
                        ...
                        >
                          ...
                          >
                            <path
                              strokeLinecap="round"
                              strokeLinejoin="round"
                              d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0011.664 0l3.181-3.183m-4.991-2.691V5.25a3.375 3.375 0 00-3.375-3.375H8.25a3.375 3.375 0 00-3.375 3.375v2.25"
                            />
                          </svg>
                        </motion.div>
                      ) : (
                ...
                <button
                  ...
                  disabled={
                    !isLoading &&
                    (!hasInput || logic.isEnhancing || isProcessingFiles)
                  }
                  aria-label={
                    isLoading ? 'Stop generating' : 'Send message'
                  }
                  className={`${sendButtonClasses} ${sendButtonStateClasses} overflow-hidden`}
                >
                  {isLoading ? (
                    <dotlottie-wc
                      src="https://lottie.host/1391f64b-e104-495f-81c1-eb74d1dafb34/0KpaEgMAuh.lottie"
                      loop
                      autoplay
                      className="w-14 h-14"
                    ></dotlottie-wc>
                    ...
                    >
                      <g transform="matrix(0.8, 0, 0, 0.8, 51.2, 56.2)">
                        <path
                          d="M481.508 210.335 68.414 38.926c-17.403-7.222-37.063-4.045-51.309 8.287C2.859 59.547-3.099 78.55 1.557 96.808L42.151 256 1.557 415.192c-4.656 18.258 1.301 37.261 15.547 49.595 14.273 12.358 33.938 15.495 51.31 8.287l413.094-171.409C500.316 293.861 512 276.363 512 256s-11.684-37.861-30.492-45.665zm-11.499 63.62L56.916 445.364c-6.947 2.881-14.488 1.665-20.175-3.259-5.686-4.923-7.971-12.212-6.113-19.501L69.287 271h149.065c8.285 0 15.001-6.716 15.001-15.001s-6.716-15.001-15.001-15.001H69.288L30.628 89.396c-1.858-7.288.427-14.578 6.113-19.501 5.686-4.923 13.225-6.141 20.174-3.259l413.094 171.409c11.125 4.616 11.99 14.91 11.99 17.955s-.865 13.339-11.99 17.955z"
                          fill="currentColor"
                        />
                      </g>
                    </svg>
