...
export const McqComponent: React.FC<McqComponentProps> = ({ question, options, answer, explanation }) => {
  ...
  return (
    <motion.div
      ...
      className="my-4 p-4 sm:p-6 bg-[#202123] rounded-2xl text-slate-200 max-w-full w-full"
      role="region"
      aria-label="Multiple Choice Question"
    >
      <p className="font-semibold text-slate-100 mb-6 text-base leading-relaxed">{question}</p>
      <div className="flex flex-col gap-3">
        {options.map((option, index) => {
          const isSelected = selectedOption === option;
          const isCorrectAnswer = option === answer;
          
          let optionClasses = 'bg-[#2d2d2d] border border-transparent hover:bg-[#3c3c3c]';
          if (isAnswered) {
             if (isSelected && isCorrectAnswer) {
                ...
                optionClasses = 'bg-[#2d2d2d] border border-transparent opacity-50';
             }
          }
          ...
          return (
            <motion.button
              ...
              className={`text-left p-4 rounded-xl transition-all duration-300 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-[#202123] focus:ring-teal-500 ${optionClasses} ${isAnswered ? 'cursor-default' : 'cursor-pointer'}`}
              layout
            >
                <div className="flex items-start gap-4">
                    <span className="font-medium text-slate-400 mt-0.5">{getOptionLetter(index)}.</span>
                    <div className="flex-1 min-w-0">
                        <p className="font-medium text-slate-200 break-words">{option}</p>
                        <AnimatePresence>
                                ...
                                >
                                    <div className="flex items-start gap-3">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-sm text-slate-300 mt-1.5 leading-relaxed break-words">{explanation}</p>
                                        </div>
                                    </div>
                                </motion.div>
                            )}
