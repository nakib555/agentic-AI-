      ...
      {isOpen && (
        ...
        >
          <motion.div
            ...
            className="bg-ui-100 rounded-2xl shadow-xl w-full max-w-md border border-color flex flex-col max-h-[90vh]"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Header */}
            <div className="flex items-start gap-4 p-4 border-b border-color flex-shrink-0">
              ...
              <div className="flex-1 min-w-0">
                <h2 id="memory-confirm-title" className="text-lg font-bold text-text-primary">
                  Save to Memory?
                </h2>
                ...
                <ul className="space-y-3">
                    {suggestions.map((suggestion, index) => (
                        <motion.li 
                            ...
                            className="flex items-start gap-3 p-3 bg-ui-200 rounded-lg text-sm"
                        >
            ...
            {/* Footer */}
            <div className="flex items-center justify-end gap-3 p-4 border-t border-color flex-shrink-0">
              <button
                onClick={onCancel}
                className="px-4 py-2 text-sm font-semibold text-text-primary bg-ui-200 hover:bg-ui-300 rounded-lg transition-colors"
              >
                Don't Save
              </button>
              <button
                onClick={onConfirm}
                className="px-4 py-2 text-sm font-semibold text-primary-content bg-primary hover:bg-primary-focus rounded-lg transition-colors"
              >
                Save to Memory
              </button>
            </div>
