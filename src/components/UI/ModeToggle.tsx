    ...
    <div className={`flex items-center gap-1 p-1 rounded-full bg-gray-200/70 dark:bg-black/20 ${disabled ? 'opacity-50' : ''}`}>
      {modes.map(mode => (
        <button
          ...
          disabled={disabled}
          className="relative px-3 py-1.5 text-sm font-semibold transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 rounded-full"
          aria-pressed={isAgentMode === mode.isAgent}
        >
          {isAgentMode === mode.isAgent && (
            <motion.div
              layoutId="mode-pill"
          ...
          <span className={`relative z-10 flex items-center gap-2 ${isAgentMode === mode.isAgent ? 'text-gray-900 dark:text-slate-50' : 'text-gray-500 dark:text-slate-400'}`}>
            {mode.icon}
            {mode.label}
          </span>
        </button>
