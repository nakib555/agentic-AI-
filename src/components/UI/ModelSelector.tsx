  ...
  return (
    <div className={`relative ${className}`} ref={selectorRef}>
      <button
        ...
        className="px-3 py-2 w-full border border-color rounded-lg shadow-sm bg-ui-100/60 backdrop-blur-md flex items-center justify-between gap-2 disabled:opacity-60 disabled:cursor-not-allowed"
      ...
      >
        <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-sm text-text-primary">{disabled ? 'Loading Models...' : selectedModelName}</span>
        </div>
        <motion.div animate={{ rotate: isOpen ? 180 : 0 }}>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5 text-text-muted"><path fillRule="evenodd" d="M5.22 8.22a.75.75 0 0 1 1.06 0L10 11.94l3.72-3.72a.75.75 0 1 1 1.06 1.06l-4.25 4.25a.75.75 0 0 1-1.06 0L5.22 9.28a.75.75 0 0 1 0-1.06Z" clipRule="evenodd" /></svg>
        </motion.div>
      </button>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.ul
            ...
            exit={{ opacity: 0, y: -10 }}
            className="absolute top-full right-0 mt-2 w-full sm:w-64 bg-ui-100/80 backdrop-blur-lg border border-color rounded-lg shadow-lg z-10 overflow-hidden"
            role="listbox"
          >
            {models.map(model => (
              <li key={model.id} className="group relative">
                <button
                  ...
                  className={`w-full text-left px-4 py-2 text-sm flex items-center justify-between ${selectedModel === model.id ? 'bg-primary/10 text-primary-focus' : 'text-text-secondary hover:bg-ui-200'}`}
                  role="option"
