  ...
  return (
    <div className={`relative ${className}`} ref={selectorRef}>
      <button
        ...
        className="px-3 py-2 w-full border border-white/40 dark:border-white/10 rounded-lg shadow-sm bg-white/30 dark:bg-black/20 backdrop-blur-xl flex items-center justify-between gap-2 disabled:opacity-60 disabled:cursor-not-allowed"
        aria-haspopup="listbox"
        aria-expanded={isOpen}
      >
        <div className="flex items-center gap-2">
            <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 text-cyan-600 dark:text-cyan-400">
            ...
            {models.map(model => (
              <li key={model.id} className="group relative">
                <button
                  ...
                  className={`w-full text-left px-4 py-2 text-sm flex items-center justify-between ${selectedModel === model.id ? 'bg-cyan-100 text-cyan-800 dark:bg-cyan-500/20 dark:text-cyan-300' : 'text-slate-700 hover:bg-slate-100/80 dark:text-slate-300 dark:hover:bg-white/5'}`}
                  role="option"
