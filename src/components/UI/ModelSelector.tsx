    ...
    <div className={`relative ${className}`} ref={selectorRef}>
      ...
      >
        <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-base text-slate-800 dark:text-slate-100">{disabled ? 'Loading Models...' : selectedModelName}</span>
        </div>
            ...
            {models.map(model => (
              <li key={model.id} className="group relative">
                <button
                  ...
                  className={`w-full text-left px-4 py-2 text-base flex items-center justify-between ${selectedModel === model.id ? 'bg-indigo-100 text-indigo-800 dark:bg-indigo-500/20 dark:text-indigo-300' : 'text-slate-700 hover:bg-slate-100/80 dark:text-slate-300 dark:hover:bg-white/5'}`}
                  role="option"
