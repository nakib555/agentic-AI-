    ...
    <div className="relative w-full sm:w-auto" ref={selectorRef}>
      ...
      >
        <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-sm">{disabled ? 'Loading Models...' : selectedModelName}</span>
        </div>
            ...
            {models.map(model => (
              <li key={model.id} className="group relative">
                <button
                  ...
                  className={`w-full text-left px-4 py-2 text-sm flex items-center justify-between ${selectedModel === model.id ? 'bg-teal-100 text-teal-700 dark:bg-teal-500/10 dark:text-teal-300' : 'text-slate-700 hover:bg-slate-100/80 dark:text-slate-300 dark:hover:bg-white/5'}`}
                  role="option"
