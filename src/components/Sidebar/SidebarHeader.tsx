  ...
  return (
    <div className={`flex items-center mb-6 h-10 ${shouldCollapse ? 'justify-center' : 'justify-between'}`}>
      <div className="relative group">
          <button 
              onClick={onNewChat}
              className={`w-full flex items-center gap-2 rounded-lg transition-colors text-left bg-violet-100/60 dark:bg-violet-900/30 hover:bg-violet-100 dark:hover:bg-violet-900/50 ${shouldCollapse ? 'p-2.5 justify-center' : 'px-3 py-2'}`}
          >
              ...
              <motion.span 
                  className="font-semibold text-base overflow-hidden text-slate-800 dark:text-slate-100"
                  initial={false}
