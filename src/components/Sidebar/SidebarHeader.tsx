  ...
  return (
    <div className={`flex items-center mb-6 h-10 ${shouldCollapse ? 'justify-center' : 'justify-between'}`}>
      <div className="relative group">
          <button 
              onClick={onNewChat}
              className={`w-full flex items-center gap-2 rounded-lg transition-colors text-left bg-ui-200/60 hover:bg-ui-200/100 ${shouldCollapse ? 'p-2.5 justify-center' : 'px-3 py-2'}`}
          >
              <div className="w-6 h-6 rounded-full bg-gradient-to-br from-secondary to-accent flex items-center justify-center flex-shrink-0">
              ...
              <motion.span 
                  className="font-semibold text-sm overflow-hidden text-text-primary"
                  initial={false}
