    ...
    return (
        <div className="relative group">
            <button 
                ...
                disabled={isEditing}
                className={`w-full text-sm p-2 rounded-lg text-left flex items-center gap-3 transition-colors ${active ? 'bg-indigo-600 text-white font-semibold dark:bg-indigo-500' : 'text-slate-600 hover:bg-gray-100/60 dark:text-slate-300 dark:hover:bg-violet-900/30 dark:hover:text-slate-100'} ${shouldCollapse ? 'justify-center' : ''} ${!shouldCollapse ? 'pr-8' : ''}`}
            >
                {isLoading && (
