...
export const VirtualizedCodeViewer: React.FC<VirtualizedCodeViewerProps> = ({ content, language }) => {
    ...
    return (
        <div className="h-full w-full bg-code-surface text-code-text font-mono text-[13px] overflow-y-auto custom-scrollbar p-4">
            {lines.map((line, index) => (
                <div key={index} className="flex leading-6 hover:bg-black/5 dark:hover:bg-white/5 transition-colors group">
                    {/* Line Number */}
                    <span 
                        className="w-12 flex-shrink-0 text-right pr-4 text-slate-400 dark:text-slate-600 select-none border-r border-slate-200 dark:border-white/5 mr-4 bg-transparent"
