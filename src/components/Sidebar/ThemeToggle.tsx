...
export const ThemeToggle = ({ theme, setTheme, isCollapsed, isDesktop }: { theme: Theme, setTheme: (theme: Theme) => void, isCollapsed: boolean, isDesktop: boolean }) => {
    ...
    return (
        <div className={`p-1 rounded-lg bg-ui-200/60 flex transition-all ${shouldCollapse ? 'flex-col gap-1' : 'justify-between'}`}>
            {buttons.map(btn => (
                <div key={btn.value} className="relative group">
                    <button
                        onClick={() => setTheme(btn.value as Theme)}
                        className={`w-full flex-1 flex items-center gap-2 p-1.5 rounded-md text-sm transition-colors ${theme === btn.value ? 'bg-ui-100 text-text-primary shadow-sm' : 'text-text-muted hover:text-text-primary'} ${shouldCollapse ? 'justify-center' : ''}`}
