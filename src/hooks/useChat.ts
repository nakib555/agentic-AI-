  ...
  const sendMessage = async (userMessage: string, files?: File[], options: { isHidden?: boolean, isThinkingModeEnabled?: boolean } = {}) => {
    ...
    const toolExecutor = async (name: string, args: any): Promise<string> => {
        ...
        if (name === 'executeCode' && finalArgs.input_filenames && Array.isArray(finalArgs.input_filenames)) {
            ...
            const lastUserMessage = currentChat?.messages.filter(m => m.role === 'user' && !m.isHidden).pop();

            if (lastUserMessage?.attachments) {
                const attachmentsToLoad = lastUserMessage.attachments.filter(
                    att => finalArgs.input_filenames.includes(att.name)
                );
                
                if (attachmentsToLoad.length > 0) {
                    finalArgs.input_files = attachmentsToLoad.map(att => ({
                        filename: att.name,
