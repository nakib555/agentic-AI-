...
export const parseMessageText = (text: string, isThinking: boolean, hasError: boolean): ParseResult => {
  ...
  // Rule 3: If model is still thinking
  if (isThinking) {
    ...
    // Removed buffering logic here to ensure immediate feedback for standard chat responses.
    // Previously, we waited if text started with '[' to avoid partial tags, but this caused 
    // blank messages for markdown links or generic bracketed text at the start.
